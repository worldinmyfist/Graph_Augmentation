...
def gen_graphs(args, dl, vgae):
    ...
    # sp.csr_matrix
    if args.gae:
        pickle.dump(adj_orig, open(f'{BASE_DIR}/data/graphs/{args.dataset}_graph_0_gae.pkl', 'wb'))
    else:
        pickle.dump(adj_orig, open(f'{BASE_DIR}/data/graphs/{args.dataset}_graph_0.pkl', 'wb'))
    # sp.lil_matrix
    pickle.dump(dl.features_orig, open(f'{BASE_DIR}/data/graphs/{args.dataset}_features.pkl', 'wb'))
    features = dl.features.to(args.device)
    for i in range(args.gen_graphs):
        ...
        # np.ndarray
        if args.gae:
            filename = f'{BASE_DIR}/data/graphs/{args.dataset}_graph_{i+1}_logits_gae.pkl'
        else:
            filename = f'{BASE_DIR}/data/graphs/{args.dataset}_graph_{i+1}_logits.pkl'
